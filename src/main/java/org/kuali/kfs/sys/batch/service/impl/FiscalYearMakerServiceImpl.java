...
public class FiscalYearMakerServiceImpl implements FiscalYearMakerService {
    ...
    public void runProcess() {
        ...
        boolean overRideObjectCodeObjects = parameterService.getIndicatorParameter(FiscalYearMakerStep.class, KFSConstants.ChartApcParms.FISCAL_YEAR_MAKER_REPLACE_OBJECT_CODE_OBJECTS);
        
        LOG.info(String.format("parmBaseYear: %s , replaceMode: %s , overRideObjectCodeObjects: %s", parmBaseYear, replaceMode, overRideObjectCodeObjects));
        ...
        for (FiscalYearMaker fiscalYearMakerHelper : deleteList) {
            if (fiscalYearMakerHelper.isAllowOverrideTargetYear()) {
            	if (replaceMode) {
            		...
            		//we didn't delete all data, so we may want to delete all data first
            		//Per KFSPTS-2938 we want to delete ObjectCode, ObjectCodeExtension, and SubObjectCode and recreate them every time FYM runs
            		if (overRideObjectCodeObjects && (fiscalYearMakerHelper.getBusinessObjectClass() == ObjectCode.class || 
            				fiscalYearMakerHelper.getBusinessObjectClass() == SubObjectCode.class)) {
            			fiscalYearMakersDao.deleteNewYearRows(baseYear, fiscalYearMakerHelper);
            		}
            	}
            }
