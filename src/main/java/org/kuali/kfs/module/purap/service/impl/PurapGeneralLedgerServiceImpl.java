...
public class PurapGeneralLedgerServiceImpl implements PurapGeneralLedgerService {
    ...
    protected int getNextAvailableSequence(String documentNumber) {
        ...
        List <GeneralLedgerPendingEntry> glpes = (List <GeneralLedgerPendingEntry>)businessObjectService.findMatching(GeneralLedgerPendingEntry.class, fieldValues);
            ...
            if (preq.getGeneralLedgerPendingEntries() == null || preq.getGeneralLedgerPendingEntries().size() < 2) {
                ...
                if ( CREATE_PAYMENT_REQUEST.equals(processType) ) {
                    getPaymentMethodGeneralLedgerPendingEntryService().generatePaymentMethodSpecificDocumentGeneralLedgerPendingEntries(
                            preq,preq.getPaymentMethodCode(),preq.getBankCode(), KNSConstants.DOCUMENT_PROPERTY_NAME + "." + "bankCode", preq.getGeneralLedgerPendingEntry(0), false, false, sequenceHelper);
                } else if ( MODIFY_PAYMENT_REQUEST.equals(processType) ) {
                    ...
                    getPaymentMethodGeneralLedgerPendingEntryService().generatePaymentMethodSpecificDocumentGeneralLedgerPendingEntries(
                            preq,preq.getPaymentMethodCode(),preq.getBankCode(), KNSConstants.DOCUMENT_PROPERTY_NAME + "." + "bankCode", preq.getGeneralLedgerPendingEntry(0), true, false, sequenceHelper, bankOffsetAmount, changesByChart );
                }
            }
