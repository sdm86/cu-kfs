...
public class PaymentRequestAction extends AccountsPayableActionBase {
    ...
    protected void createDocument(KualiDocumentFormBase kualiDocumentFormBase) throws WorkflowException {
        ...
        ((PaymentRequestForm) kualiDocumentFormBase).setWireChargeMessage(retrieveWireChargeMessage());
...
//    }
    ...
    public ActionForward approve(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if (requiresCalculateTax((PaymentRequestForm)form)) {
            ...
            return mapping.findForward(KFSConstants.MAPPING_BASIC);
        } else if (StringUtils.equals(preq.getStatusCode(), PaymentRequestStatuses.PAYMENT_METHODL_REVIEW) && !((PaymentRequestForm)form).isCalculated()) {
            // KFSPTS-2578 : if this is optional, then don't enforce it yet
//        	GlobalVariables.getMessageMap().putError(KFSConstants.DOCUMENT_ERRORS, PurapKeyConstants.ERROR_APPROVE_REQUIRES_CALCULATE);
//            return mapping.findForward(KFSConstants.MAPPING_BASIC);
        ...
        if (SpringContext.getBean(KualiRuleService.class).applyRules(new AttributedPreCalculateAccountsPayableEvent(preq))) {
            ...
            if (StringUtils.equals(preq.getStatusCode(), PaymentRequestStatuses.PAYMENT_METHODL_REVIEW) || StringUtils.equals(preq.getStatusCode(), PaymentRequestStatuses.AWAITING_TAX_REVIEW)) {
