...
public class LaborLedgerKualiBatchFileAdminAction extends KualiBatchFileAdminAction {
    ...
    /**
     * Calls service to create a disencumbrance file.
     * @param mapping
     * @param form
     * @param request
     * @param response
    ...
    public ActionForward disencumber(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if (!SpringContext.getBean(BatchFileAdminAuthorizationService.class).canDelete(batchFile, GlobalVariables.getUserSession().getPerson())) {
            throw new RuntimeException("Error: not authorized to create a disencumbrance file");
        }
        ...
        if (question == null) {
            ...
            return performQuestionWithoutInput(mapping, fileAdminForm, request, response, "confirmDisencumber", questionText, KNSConstants.CONFIRMATION_QUESTION, "disencumber", fileAdminForm.getFilePath());
        } else {
            Object buttonClicked = request.getParameter(KFSConstants.QUESTION_CLICKED_BUTTON);
            if ("confirmDisencumber".equals(question)) {
                String status = null;
