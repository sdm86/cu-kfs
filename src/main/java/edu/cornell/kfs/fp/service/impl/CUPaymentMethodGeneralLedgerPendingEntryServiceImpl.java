...
public class CUPaymentMethodGeneralLedgerPendingEntryServiceImpl implements CUPaymentMethodGeneralLedgerPendingEntryService {
    ...
    public void generateFinalEntriesForPRNC(PaymentRequestDocument document) {
        ...
        int nextAvailableSequence = getNextAvailableSequence(document.getDocumentNumber());
        for (String offsetObjectCode : chartOffsets.values()) {
            ...
            if (glEntries != null && glEntries.size() > 0) {
                for (Entry entry : glEntries) {
                    ...
                    GeneralLedgerPendingEntrySequenceHelper sequenceHelper = new GeneralLedgerPendingEntrySequenceHelper(nextAvailableSequence);
                    ...
                    document.getGeneralLedgerPendingEntries().add(glpe);
                    // create cash entry
                    GeneralLedgerPendingEntry cashGlpe = new GeneralLedgerPendingEntry();
                    sequenceHelper = new GeneralLedgerPendingEntrySequenceHelper(nextAvailableSequence);
                    nextAvailableSequence++;
                    ...
                    document.getGeneralLedgerPendingEntries().add(cashGlpe);
